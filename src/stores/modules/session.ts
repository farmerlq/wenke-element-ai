...
export const useSessionStore = defineStore('session', () => {
  ...
  const requestSessionList = async (page: number = currentPage.value, force: boolean = false) => {
    ...
    try {
      ...
      // 预处理会话分组 并添加前缀图标
      const rows = resArr?.data?.rows || [];
      // 确保字段映射正确
      const normalizedRows = rows.map((row: any) => ({
        ...row,
        id: row.id || row.session_id,
        sessionTitle: row.sessionTitle || row.session_name,
        userId: row.userId || row.user_id,
        createTime: row.createTime || row.created_at,
      }));
      const res = processSessions(normalizedRows);
  ...
  const createSessionList = async (data: Omit<CreateSessionDTO, 'id'>) => {
    ...
    try {
      ...
      const targetIndex = sessionList.value.findIndex(session => session.id === `${res.data.sessionId}`);
      ...
      // 并将当前勾选信息设置为新增的会话信息
      const newSessionRes = await get_session(`${res.data.sessionId}`);
